class RoomsController < ApplicationController
  ...
  def index
    ...
    @recent_activities = RecentActivity.where(subject_type: "User").where(subject_id: current_user.friend_ids.push(current_user.id)).order("created_at DESC")
  ...
  def search_friend
    ...
    @users=User.joins(:profile).where("lower(concat(first_name,' ',last_name)) like ?","%#{@term.downcase}%")
  ...
  def search_friend_all_results
    ...
    @users=[]
    @top_users = User.joins(:profile).order("gp DESC").limit(5)
    @users=User.joins(:profile).where("lower(concat(first_name,' ',last_name)) like ?","%#{term.downcase}%") if term
  end
end
